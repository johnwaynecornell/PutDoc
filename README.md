...
1.  Install .NET 8 SDK.
2.  Clone this repo.
3.  Set the desired working directory in `appsettings.local.json` (`PutDocRootPath`), or pass via environment/config/comman line.
    *   Sample `appsettings.local.json` to be placed next to `wwwroot`
        
        ```
        {
          "PutDocRootPath": "/real/path/here/PutDoc" 
        }
        ```
